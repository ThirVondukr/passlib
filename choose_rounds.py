...
def main(*args):
    ...
    # ---------------------------------------------------------------
    rounds = clamp_rounds(cost_to_rounds(0.5 * rounds_to_cost(hasher.default_rounds)))
        ...
        if (target - lower_elapsed) / target < tolerance:
            ...
            print(
                ...
                % (
                    ...
                    lower_elapsed * 1000,
                    faster * 1000,
                    round(100 * faster / target),
                )
            )
            slower = upper_elapsed - target
            print(
                "target rounds...: %d (%dms -- %dms/%d%% slower than requested)"
                % (
                    upper,
                    upper_elapsed * 1000,
                    slower * 1000,
                    round(100 * slower / target),
                )
            )
    else:
